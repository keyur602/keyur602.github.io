<?php
    ...
    if (!empty($_GET['ad_id'])) 
    {
        ...
        $que="select * from photo where `photo_id`='$id'";
        ...
        $q="delete from photo where `photo_id`='$id'"; 
        mysqli_query($con,$q);
    }
    if (!empty($_GET['sdid'])) 
    {   
        ...
        {
           $id = $_GET['sdid'];
           $st="update photo set `status`='1' where `photo_id`='$id'";
           mysqli_query($con,$st);
        }
        else if ($_GET['status']=='acitve')
        {
            $id = $_GET['sdid'];
            $st="update photo set `status`='0' where `photo_id`='$id'";
            mysqli_query($con,$st);   
        }
    }
                                        ...
                                        <?php while ($deta = mysqli_fetch_array($query1)) { ?>
                                            <tr>
                                                ...
                                                <td>
                                                   ...
                                                   <a class="btn btn-danger btn-sm" href="photos-data.php?status=deacitve&sdid=<?php echo $deta['photo_id']?>">deacitve</a>
                                                   <?php } else{ ?>
                                                   <a class="btn btn-primary btn-sm" href="photos-data.php?status=acitve&sdid=<?php echo $deta['photo_id']?>">acitve</a>
                                                   <?php } ?>

                                                </td>
                                                <td>
                                                    <a class="btn btn-danger btn-sm" href="photos-data.php?ad_id=<?php echo $deta['photo_id']; ?>">Delete</a>
                                                    <a class="btn btn-primary btn-sm" href="photos-forms.php?ad_id=<?php echo $deta['photo_id']; ?>">Update</a>
                                                </td>
                                            </tr>
                                        <?php } ?>
                                    </tbody>
