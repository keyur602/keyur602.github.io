<?php
    ...
    if (isset($_POST['signin'])) 
    {
        ...
        $email=$_POST['email'];
        $forg = "select * from admin_data where `email`='$email'";
        $forg1 = mysqli_query($con,$forg);
        $num = mysqli_num_rows($forg1);
        if ($num == 1) {
            
            $fordeta = mysqli_fetch_array($forg1);
            $_SESSION['email'] = $fordeta['email'];
