<?php
    ...
    if (!empty($_GET['sdid'])) 
    {
        ...
        {
           $id = $_GET['sdid'];
           $st="update category set `status`='1' where `category_id`='$id'";
           mysqli_query($con,$st);
        }
        else if(!empty($_GET['status']=='acitve'))
        {
            $id = $_GET['sdid'];
            $st="update category set `status`='0' where `category_id`='$id'";
            mysqli_query($con,$st);   
        }
    }
                                        ...
                                        <?php while ($deta = mysqli_fetch_array($query1)) { ?>
                                            <tr>
                                                ...
                                                <td>
                                                   ...
                                                   <a class="btn btn-danger btn-sm" href="category_table.php?status=deacitve&sdid=<?php echo $deta['category_id']?>">deacitve</a>
                                                   <?php } else{ ?>
                                                   <a class="btn btn-primary btn-sm" href="category_table.php?status=acitve&sdid=<?php echo $deta['category_id']?>">acitve</a>
                                                   <?php } ?>

                                                </td>
                                                <td>
                                                    <a class="btn btn-danger btn-sm" href="category_table.php?ad_id=<?php echo $deta['category_id']; ?>">Delete</a>
                                                    <a class="btn btn-primary btn-sm" href="category_form.php?ad_id=<?php echo $deta['category_id']; ?>">Update</a>
                                                </td>
                                            </tr>
                                        <?php } ?>
                                    </tbody>
