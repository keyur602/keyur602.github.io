<?php
    ...
    if ($par2['logtip'] != 'admin') {
        $pars = "select * from addtable where `name`='slider'";
    ...
    {   
        ...
        {
           $id = $_GET['sdid'];
           $st="update slider set `status`='1' where `slider_id`='$id'";
           mysqli_query($con,$st);
        }
        else if ($_GET['status']=='acitve')
        {
            $id = $_GET['sdid'];
            $st="update slider set `status`='0' where `slider_id`='$id'";
            mysqli_query($con,$st);   
        }
    }
                                        ...
                                        <?php while ($deta = mysqli_fetch_array($query1)) { ?>
                                            <tr>
                                                ...
                                                <td>
                                                   ...
                                                   <a class="btn btn-danger btn-sm" href="slider-data.php?status=deacitve&sdid=<?php echo $deta['slider_id']?>">deacitve</a>
                                                   <?php } else{ ?>
                                                   <a class="btn btn-primary btn-sm" href="slider-data.php?status=acitve&sdid=<?php echo $deta['slider_id']?>">acitve</a>
                                                   <?php } ?>

                                                </td>
                                                <td>
                                                    <a class="btn btn-danger btn-sm" href="slider-data.php?ad_id=<?php echo $deta['slider_id']; ?>">Delete</a>
                                                    <a class="btn btn-primary btn-sm" href="slider-forms.php?ad_id=<?php echo $deta['slider_id']; ?>">Update</a>
                                                </td>
                                            </tr>
                                        <?php } ?>
                                    </tbody>
